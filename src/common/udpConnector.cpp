...
UDPConnector::UDPConnector(int port)
    : m_sockv4(AF_INET, port), m_sockv6(AF_INET6, port) {
  m_isRunning = true;
...
int UDPConnector::sendTo(const std::string &host, const std::string &port,
  ...
  if (getaddrinfo(host.c_str(), port.c_str(), &hints, &resultList) != 0) {
    Utils::logAndThrowFatal(
        g_logger, "Send To Tracker",
        std::format("Unable to get addresultLists info for {} on port {}", host,
                    port));
  }

  for (it = resultList; it != NULL; it = it->ai_next) {
    if (it->ai_family == AF_INET6 && m_sockv6.get() != -1) {
      g_logger.debug("sendTo", "Found IPv6 address sending message over IPv6.");
      if (sendto(m_sockv6.get(), bytes.data(), bytes.size(), 0, it->ai_addr,
                 it->ai_addrlen) < 0) {
        Utils::logAndThrowFatal(g_logger, "Send To Tracker",
                                "Sending over IPv6 failed");
      }
      break;
    } else if (it->ai_family == AF_INET && m_sockv4.get() != -1) {
      g_logger.debug("sendTo", "Found IPv4 address sending message over IPv4.");
      if (sendto(m_sockv4.get(), bytes.data(), bytes.size(), 0, it->ai_addr,
                 it->ai_addrlen) < 0) {
        Utils::logAndThrowFatal(g_logger, "Send To Tracker",
                                "Sending over IPv4 failed");
      }
      break;
    }
  }
...
void UDPConnector::handleRead(int fd, std::vector<char> &buffer) {
  ...
  if (n > 0)
    g_logger.info(
        "handle Read",
        std::format("Received {} bytes from sender: {}", n, sender.ss_family));
  if (m_onReceive) {
    Packet p;
